...
class MyTestModel : public model::prob_grad_ad
{
public:
	MyTestModel(double mu, double sigma)
		: model::prob_grad_ad(1), mean(mu), stddev(sigma) {}
	agrad::var log_prob(
		std::vector<agrad::var>& params_r, 
		std::vector<int>& params_i,
		std::ostream* output_stream = 0)
	{
		agrad::var lp__(0.0);
		//lp__ += prob::normal_log(params_r[0], mean, stddev);
		lp__ += log(simference::Math::Probability::NormalDistribution<agrad::var>::Prob(params_r[0], mean, stddev));
		return lp__;
	}
